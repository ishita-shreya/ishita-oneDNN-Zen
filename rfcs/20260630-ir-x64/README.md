...
register has one of three kinds (`gpr`, `vec`, `mask`), which fixes the kind of
physical register it can later occupy. A `vec` register also carries the element
data type of the values it holds.
...
The algorithm leaves room for two refinements, listed in the order we expect to
need them.
...
from the pool: the stack pointer, the kernel-argument pointer, and a few scratch
registers (GPR and vector) that the emitter uses to load and store spilled
values.
...
The emitter invokes it when it lowers `inject_postops`. The injector saves and
restores the registers it uses, so it does not participate in IR allocation, and
its constant table is written after the postamble alongside the other static
data.
