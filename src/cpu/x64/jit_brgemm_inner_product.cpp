...
void copy_data_chunk(ker_type &ker, char *tr_data, const char *data,
        dim_t os_work, bool is_last_blk) {
...
status_t brgemm_inner_product_fwd_t<isa>::execute_forward(
    ...
    const auto ker = [= COMPAT_THIS_CAPTURE](dim_t ithr_oc_mb,
                             dim_t nthr_oc_mb, dim_t ithr_ic, dim_t osb,
                             dim_t osb_s, dim_t ocb,
                             dim_t ocb_s, dim_t icc, dim_t icc_s, int kd, int kh,
                             int kw, bool copy_buffer_a, dim_t &prev_ker_idx,
                             const void *dst_scales_ptr) {
        const dim_t cur_ocb = ocb + ocb_s;
        const dim_t cur_osb = osb + osb_s;
        const dim_t cur_icc = icc + icc_s;
        const dim_t n = cur_osb * jbgp.os_block;
        const dim_t ithr = nthr_oc_mb * ithr_ic + ithr_oc_mb;
        ...
        const dim_t oc = cur_ocb * jbgp.oc_block;
        ...
        char *wsp_tile = is_amx
                ...
                : nullptr;
        dim_t icb = cur_icc * jbgp.nb_ic_blocking;
    ...
    const auto init_thr_groups
            = [=](const int ithr, const int nthr, dim_t &nthr_ic,
                      dim_t &nthr_oc_mb, dim_t &ithr_ic,
                      dim_t &ithr_oc_mb) {
    ...
    parallel(static_cast<int>(num_threads), [=](const int ithr, const int nthr) {
        dim_t nthr_ic {1}, nthr_oc_mb {1}, ithr_ic {0}, ithr_oc_mb {0};
        ...
        dim_t icc_start {0}, icc_end {ic_chunks};
        if (nthr_ic > 1)
        ...
        while (work < end) {
            ...
            dim_t ocb_e = nstl::min(ocb_s + jbgp.nb_oc_blocking, jbgp.nb_oc);
            dim_t ocb_work = ocb_e - ocb_s;

            dim_t osb_s = osc * jbgp.nb_os_blocking;
            dim_t osb_e = nstl::min(osb_s + jbgp.nb_os_blocking, jbgp.nb_os);
    ...
    if (jbgp.nthr_ic_b > 1) {
        const auto get_dst_reduced_off = [=](int ithr_ic, dim_t osb, dim_t ocb) {
        ...
        parallel(static_cast<int>(num_threads),
                        ...
                        for (dim_t ocb = ocb_s; ocb < ocb_e; ++ocb) {
                            const bool is_oc_tail
                                    = (jbgp.oc - ocb * jbgp.oc_block
                                            < jbgp.oc_block);
                            const dim_t brg_ker_idx = brgemm_inner_product_utils::
...
void brgemm_inner_product_bwd_data_t<isa>::execute_backward_data(
    ...
    const dim_t oc_chunks = div_up(jbgp.nb_oc, jbgp.nb_oc_blocking);
    const dim_t os_chunks = div_up(jbgp.nb_os, jbgp.nb_os_blocking);
    const dim_t work_amount = jbgp.nb_ic * jbgp.ks() * os_chunks;
    const dim_t num_threads
    ...
    const auto get_weights_ptr
        ...
        dim_t fwd_oc_block = jbgp.get_weights_oc_block();
        dim_t ic = icb * jbgp.ic_block;
        dim_t oc = ocb * jbgp.oc_block;
    ...
    const auto transform_b_chunk
            = [= COMPAT_THIS_CAPTURE](char *tr_wei, const char *wei,
                      dim_t trans_batch, dim_t current_N, dim_t current_K) {
    ...
    const auto ker = [= COMPAT_THIS_CAPTURE](dim_t ithr_ic_mb,
                             dim_t nthr_ic_mb, dim_t ithr_oc, dim_t nthr_oc,
                             dim_t n, dim_t icb, dim_t occ,
                             int kd, int kh, int kw, bool do_init,
                             bool do_b_transpose, dim_t &prev_ker_idx) {
        const dim_t ithr = nthr_ic_mb * ithr_oc + ithr_ic_mb;
        brgemm_batch_element_t *addr_batch
                = addr_batch_global + ithr * jbgp.adjusted_batch_size;

        const dim_t ic = icb * jbgp.ic_block;
        const dim_t ocb = occ * jbgp.nb_oc_blocking;
        const dim_t oc = ocb * jbgp.oc_block;
    ...
    if (jbgp.global_b_transpose && jbgp.use_buffer_b) {
        ...
        parallel(static_cast<int>(num_threads), [=](const int ithr, const int nthr) {
            dim_t start {0}, end {0};
            dim_t max_ch_block = nstl::max(jbgp.ic_block, jbgp.oc_block);
            dim_t ic_chunk_sz = max_ch_block / jbgp.ic_block;
            dim_t oc_chunk_sz = max_ch_block / jbgp.oc_block;
            dim_t nc_ic = div_up(jbgp.nb_ic, ic_chunk_sz);
            dim_t nc_oc = div_up(jbgp.nb_oc, oc_chunk_sz);
            dim_t transp_work_amount = nc_ic * nc_oc;
            ...
            while (start < end) {
                ...
                dim_t icb_end = nstl::min<dim_t>((icc + 1) * ic_chunk_sz, jbgp.nb_ic);
                dim_t ocb_start = occ * oc_chunk_sz;
                dim_t ocb_end = nstl::min<dim_t>((occ + 1) * oc_chunk_sz, jbgp.nb_oc);
    ...
    parallel(static_cast<int>(num_threads), [=](const int ithr, const int nthr) {
        const dim_t nthr_oc = jbgp.nthr_oc_b <= nthr ? jbgp.nthr_oc_b : 1;
        const dim_t nthr_ic_mb = nthr / nthr_oc;
        const dim_t ithr_ic_mb = ithr % nthr_ic_mb;
        const dim_t ithr_oc = ithr / nthr_ic_mb;
        ...
        dim_t occ_start {0}, occ_end {oc_chunks};
        if (nthr_oc > 1)
        ...
        while (work < end) {
            const dim_t nb_os_blocking
                    = nstl::min(jbgp.nb_os - osc * jbgp.nb_os_blocking,
                            jbgp.nb_os_blocking);
            const dim_t occ_work = occ_end - occ_start;
            const dim_t loop_iteration = nb_os_blocking * occ_work;

            for (int iter = 0; iter < loop_iteration; ++iter) {
                dim_t osb = 0, occ = occ_start;
                ...
                dim_t n = (osc * jbgp.nb_os_blocking + osb) * jbgp.os_block;
    ...
    if (jbgp.nthr_oc_b > 1) {
        parallel(static_cast<int>(num_threads),
                [= COMPAT_THIS_CAPTURE](const int ithr, const int nthr) {
            const dim_t nthr_oc = jbgp.nthr_oc_b <= nthr ? jbgp.nthr_oc_b : 1;
            const dim_t n_oc_bufs = nstl::min<dim_t>(oc_chunks, nthr_oc);
            ...
            dim_t n_accs = n_oc_bufs - oc_buf_end;
...
struct brgemm_inner_product_bwd_weights_t<isa>::thread_info_t {
    ...
    int ithr;
    dim_t ithr_sp_ic_c, ithr_oc_c, ithr_os_c;
    dim_t nthr;
    dim_t nthr_sp_ic_c, nthr_oc_c, nthr_os_c;

    dim_t os_c_start = 0, os_c_end = 0, os_c_work;
    dim_t oc_c_start = 0, oc_c_end = 0, oc_c_work;
    dim_t sp_ic_c_start = 0, sp_ic_c_end = 0, sp_ic_c_work;
    ...
    thread_info_t(const brgemm_inner_product_bwd_weights_t *self,
        ...
        dim_t ic_chunks = div_up(jbgp.nb_ic, jbgp.nb_ic_blocking);
        dim_t os_chunks = div_up(jbgp.nb_os, jbgp.nb_os_blocking);
        dim_t sp_ic_chunks = ic_chunks * jbgp.ks();
    ...
    char *get_buffer_a_ptr(dim_t sp_icb, dim_t osc) const {
        if (!buffer_a_) return (char *)nullptr;

        dim_t icb_idx = sp_icb % (sp_ic_chunks_per_thread_ * nb_ic_blocking_);
        dim_t osc_idx = thread_local_input_buffers_ ? 0 : osc - os_c_start;

        return buffer_a_ + osc_idx * buffer_a_osc_shift_
                + icb_idx * buffer_a_icb_shift_;
    }

    char *get_buffer_b_ptr(dim_t ocb, dim_t osc) const {
        if (!buffer_b_) return (char *)nullptr;

        const dim_t ocb_idx = ocb % nb_oc_blocking_;
        const dim_t osc_idx = thread_local_input_buffers_ ? 0 : osc - os_c_start;
    ...
    dim_t ic_chunks() const { return ic_chunks_; }
...
private:
    ...
    dim_t nb_ic_blocking_ = 1;
    dim_t nb_oc_blocking_ = 1;
    ...
    dim_t ic_chunks_ = 0;
    dim_t sp_ic_chunks_per_thread_ = 0;
};

template <cpu_isa_t isa>
void brgemm_inner_product_bwd_weights_t<isa>::transform_matrix_a_chunk(
        char *tr_src, const char *src, dim_t trans_batch, dim_t current_m,
        dim_t current_k) const {
...
void brgemm_inner_product_bwd_weights_t<isa>::transform_matrix_b_chunk(
        char *tr_diff_dst, const char *diff_dst, dim_t trans_batch,
        dim_t current_col_size, dim_t current_row_size) const {
...
void brgemm_inner_product_bwd_weights_t<isa>::transpose_matrix_c_chunk(
        const thread_info_t *ti, const dim_t ocb, const dim_t icb,
        dim_t oc_size, dim_t ic_size, dim_t kd, dim_t kh, dim_t kw,
...
char *brgemm_inner_product_bwd_weights_t<isa>::get_wei_acc_ptr(
    ...
    const dim_t buf_idx = reduction_buf_idx >= 0
            ? reduction_buf_idx
    ...
    if ((jbgp.nthr_mb > 1 && buf_idx < 0)
        ...
        const dim_t icb_scale = (!jbgp.is_amx || jbgp.wei_dt == jbgp.acc_dt)
    ...
    const dim_t ocb_l = ocb % jbgp.nb_oc_blocking;
    const dim_t icb_l = icb % jbgp.nb_ic_blocking;
...
void brgemm_inner_product_bwd_weights_t<isa>::compute_diff_weights_and_bias(
    ...
    const dim_t oc_chunk_sz = jbgp.oc_block * jbgp.nb_oc_blocking;
    ...
    dim_t os_chunks = utils::div_up(jbgp.nb_os, jbgp.nb_os_blocking);

    const auto get_bia_acc_ptr = [&](dim_t oc) {
    ...
    const auto ker
            = [&](const dim_t sp_icc, const dim_t osc, const dim_t icc, const dim_t occ,
                      const dim_t icb_i, const dim_t ocb_i, const dim_t osc_prev,
                      const dim_t sp_icc_prev, const dim_t occ_prev, int kd, int kh,
                      int kw, dim_t &prev_ker_idx) {
        brgemm_batch_element_t *addr_batch
        ...
        char *wsp_tile = is_amx_bf16
                ...
                : nullptr;
        dim_t icb = icb_i + icc * jbgp.nb_ic_blocking;
        dim_t ocb = ocb_i + occ * jbgp.nb_oc_blocking;
        dim_t ic = icb * jbgp.ic_block;
        dim_t oc = ocb * jbgp.oc_block;
        dim_t osb = osc * jbgp.nb_os_blocking;
        dim_t n = osb * jbgp.os_block;

        // Base buffer pointers for the current kernel iteration,
        // x_buf_osb_shift values are used for shifting wrt osb iter variable
        dim_t sp_icb = sp_icc * jbgp.nb_ic_blocking + icb_i;
        ...
        bool is_ic_tail = jbgp.ic - ic < jbgp.ic_block;
        bool is_oc_tail = jbgp.oc - oc < jbgp.oc_block;
        const dim_t oc_chunk_tail = jbgp.oc % oc_chunk_sz;
        const bool is_last_oc_chunk = jbgp.oc - oc < oc_chunk_sz;
        const dim_t curr_oc_chunk_sz = oc_chunk_tail > 0 && is_last_oc_chunk
                ? oc_chunk_tail
                : oc_chunk_sz;
        ...
        if (kernel_init && (is_ic_tail || is_oc_tail)) {
            ...
            dim_t last_ic_chunk = jbgp.ic_block;
        ...
        if (is_os_tail) {
            ...
            const dim_t brg_ker_idx_os_tail
                    = brgemm_inner_product_utils::get_brg_kernel_index(
    ...
    dim_t prev_ker_idx = -1;
    dim_t osc_prev = -1, sp_icc_prev = -1, occ_prev = -1;
    while (loop_idx < loop_end) {
        const dim_t occ = ti->oc_c_start + occ_idx;
        const dim_t sp_icc = ti->sp_ic_c_start + sp_icc_idx;
        const dim_t osc = ti->os_c_start + osc_idx;
        const dim_t ic_chunks = ti->ic_chunks();
        ...
        const dim_t ocb_work = nstl::min(
                jbgp.nb_oc_blocking, jbgp.nb_oc - occ * jbgp.nb_oc_blocking);
        const dim_t icb_work = nstl::min(
...
void brgemm_inner_product_bwd_weights_t<
    ...
    if (dnnl_thr_syncable() && jbgp.nthr > 1)
        simple_barrier::barrier(ti->barrier_ctx, static_cast<int>(jbgp.nthr));
    if (ti->nthr_os_c == 1) return;

    const bool is_f32_out = jbgp.wei_dt == data_type::f32;
    const dim_t icb_scale = is_f32_out ? jbgp.ic_block / jbgp.simd_w : 1;

    const dim_t icb_work = nstl::min(ti->sp_ic_c_work * jbgp.nb_ic_blocking,
            jbgp.nb_ic - ti->sp_ic_c_start * jbgp.nb_ic_blocking);
    const dim_t ocb_work = nstl::min(ti->oc_c_work * jbgp.nb_oc_blocking,
            jbgp.nb_oc - ti->oc_c_start * jbgp.nb_oc_blocking);
    const dim_t work = ocb_work * icb_work * jbgp.ks();

    dim_t os_chunks = utils::div_up(jbgp.nb_os, jbgp.nb_os_blocking);
    dim_t reduce_buffers = nstl::min<dim_t>(ti->nthr_os_c, os_chunks);
    int reduce_buf_idx_start = !is_f32_out;
    dim_t reduce_buf_idx_end = reduce_buffers - is_f32_out;
    ...
    const dim_t acc_size = jbgp.ic_block * jbgp.oc_block;
    ...
    if (jbgp.with_bias && ti->ithr_sp_ic_c == 0 && ti->sp_ic_c_work > 0
        ...
        dim_t reduce_buf_idx_end = reduce_buffers - 1;
        dim_t oc_chunk_size = jbgp.nb_oc_blocking * jbgp.oc_block;
        dim_t oc = ti->oc_c_start * oc_chunk_size;
        dim_t acc_size = nstl::min<dim_t>(ti->oc_c_work * oc_chunk_size, jbgp.oc - oc);
...
void brgemm_inner_product_bwd_weights_t<isa>::execute_backward_weights(
    ...
    parallel(static_cast<int>(jbgp.nthr),
            [= COMPAT_THIS_CAPTURE](const int ithr, const int nthr) {
