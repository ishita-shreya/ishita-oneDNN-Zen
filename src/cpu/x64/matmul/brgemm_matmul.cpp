...
dim_t get_brg_kernel_index(const brgemm_matmul_conf_t &bgmmc, bool is_bs_tail,
        bool do_initialization, dim_t m_ker_idx, dim_t n_ker_idx, bool is_K_tail,
    ...
    dim_t idx = 2 * 2 * 2 * max_n_ker_idx * max_m_ker_idx * (int)is_prefetching
...
dim_t brgemm_matmul_t<isa>::pd_t::get_brg_kernel_idx(bool is_bs_tail,
        bool do_initialization, dim_t m_ker_idx, dim_t n_ker_idx, bool is_K_tail,
...
status_t brgemm_matmul_t<isa>::pd_t::init(engine_t *engine) {
    ...
    for (int prefetching = 0; prefetching < 2; prefetching++) {
        ...
        dim_t idx = get_brg_kernel_idx(i_bs, i_init, i_M, i_N, i_K, prefetching);
        if (idx < 0) continue;
...
status_t brgemm_matmul_t<isa>::init(engine_t *engine) {
    ...
    for_(int i_init = i_init_start; i_init < 2; i_init++)
    for (int prefetching = 0; prefetching < 2; prefetching++) {
        dim_t idx = pd()->get_brg_kernel_idx(
...
status_t brgemm_matmul_t<isa>::execute_body(const exec_ctx_t &ctx) const {
    ...
    const int num_threads
            = static_cast<int>(brgmm_ctx_ptr->get_num_threads_for_parallelization());
    parallel(num_threads,
        ...
        const dim_t ithr_bmn = brgmm_ctx.get_thread_idx_for_bmn_gemm(ithr);
        const dim_t ithr_k = brgmm_ctx.get_thread_idx_for_k(ithr);
        ...
        if (brgmm_ctx.parallel_reduction_is_used())
            balance211(bgmmc.K_chunks, static_cast<dim_t>(brgmm_ctx.get_num_threads_for_k()),
...
void brgemm_matmul_t<isa>::compute_kernel(
        ...
        dim_t n_blk_idx, dim_t k_blk_idx, bool do_init, dim_t &prev_ker_idx,
        bool prefetch) const {
    const auto &bgmmc = pd()->get_brgemm_matmul_conf();
    ...
    const dim_t brg_ker_idx = pd()->get_brg_kernel_idx(
...
void brgemm_matmul_t<isa>::fill_per_mn_compensation(
        ...
        dim_t n_blk_idx, const char *A_data_batch_ptr,
...
void brgemm_matmul_t<isa>::maybe_reduce_and_convert_partial_results_A(
    ...
    parallel(num_threads,
        ...
        const auto &bgmmc = pd()->get_brgemm_matmul_conf();
        const dim_t ithr_bmn = brgmm_ctx.get_thread_idx_for_bmn(ithr);
        const dim_t ithr_k = brgmm_ctx.get_thread_idx_for_k(ithr);
        if (ithr_bmn < 0 || ithr_k < 0) return;
...
void brgemm_matmul_t<isa>::maybe_reduce_partial_results_and_apply_postops(
    ...
    parallel(num_threads,
        ...
        const auto &bgmmc = pd()->get_brgemm_matmul_conf();
        const dim_t nthr_k = brgmm_ctx.get_num_threads_for_k();
        const dim_t ithr_bmn = brgmm_ctx.get_thread_idx_for_bmn_gemm(ithr);
        const dim_t ithr_k = brgmm_ctx.get_thread_idx_for_k(ithr);
        if (ithr_bmn < 0 || ithr_k < 0) return;
        ...
        while (start < end) {
            ...
            while (mn_start < mn_end) {
                ...
                if (bgmmc.post_ops_applicable) {
                    const dim_t n_ker_idx = brgmm_ctx.get_N_kernel_idx(nb);
...
struct brgemm_matmul_t<isa>::brg_matmul_exec_ctx_t {
    brg_matmul_exec_ctx_t(
        ...
        nthr_ = nstl::min<dim_t>(dnnl_get_current_num_threads(), bgmmc.nthr);
    ...
    */
    ...
    dim_t get_base_brgemm_kernel_idx() const { return base_brg_ker_idx_; }
    ...
    dim_t get_num_threads_for_k() const { return nthr_k_; }
    bool parallel_reduction_is_used() const {
        return nthr_k_ > 1 && bgmmc_.K_chunks > 1;
    }
    dim_t get_num_threads_for_bmn() const { return nthr_bmn_; }
    ...
    dim_t get_num_threads_for_parallelization() const {
...
private:
    ...
    dim_t base_brg_ker_idx_;
    dim_t vnni_factor;

    // parallelization parameters
    dim_t parallel_work_amount_;
    dim_t parallel_work_amount_gemm_;
    dim_t nthr_, nthr_k_, nthr_bmn_, num_threads_used_;
