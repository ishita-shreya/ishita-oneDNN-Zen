...
The kernel's `generate()` runs a fixed pipeline. Only the *builder* stage is
kernel-specific. Everything below it is shared.

```mermaid
flowchart TD
    K["Kernel (jit_generator_t)<br/>knows the concrete ISA and data types"] --> B
    B["Builder<br/>emits target-neutral IR: param loads, loop nests, math"] --> RC
    RC["Register config<br/>ISA-aware step builds ISA-agnostic pools"] --> RA
    RA["Register allocator<br/>liveness and linear scan over gpr/vec/mask"] --> E
    E["Emitter<br/>lowers IR to instructions, one backend per ISA family"] --> D
    D["Static data<br/>mask and post-op tables, written after the postamble"]
```
