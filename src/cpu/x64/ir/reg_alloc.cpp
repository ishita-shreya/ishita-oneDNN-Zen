...
void alloc_file(const ir_t &ir, int file_idx,
    ...
    for (int v : scan_order) {
        ...
        // `v` now holds a register. Add it to the active set.
        active.push_back(v);
...
reg_alloc_result_t allocate_registers(
    ...
    // Step 2: collapse liveness into one [start, end] interval per vreg.
    // extend_interval(v) grows v's interval to cover operation i. A vreg
    // touches i when i defines it, uses it, or it is live on entry to i.
