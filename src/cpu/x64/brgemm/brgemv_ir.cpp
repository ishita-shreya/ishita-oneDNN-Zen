...
struct brgemv_ir_conf_t {
    ...
    int max_bs;
    int m_block, k_block;
    int dt_sz_a, dt_sz_x, dt_sz_y;
    data_type_t dt_a, dt_x, dt_y, dt_acc;
...
brgemv_ir_conf_t make_brgemv_ir_conf(const brgemm_desc_t &brg) {
    brgemv_ir_conf_t cfg {};

    cfg.m = brg.bcast_dim;
    cfg.k = brg.reduce_dim;
    cfg.lda = brg.LDA;
    cfg.incy = brg.LDC;
    cfg.max_bs = brg.brgattr.max_bs;

    cfg.m_block = brg.gemv_bd_block();
    cfg.k_block = brg.rd_block;

    cfg.dt_sz_a = brg.typesize_A;
    cfg.dt_sz_x = brg.typesize_B;
    cfg.dt_sz_y = brg.typesize_C;
    ...
    cfg.k_blocks = cfg.k / cfg.k_block;
    cfg.k_tail = cfg.k % cfg.k_block;
    cfg.mblk_a_off = cfg.dt_sz_a * cfg.m_block * cfg.lda;
    cfg.mblk_y_off = cfg.dt_sz_y * cfg.m_block * cfg.incy;
...
void build_gemv(const brgemm_desc_t &brg, ir::ir_t &ir) {
    const brgemv_ir_conf_t cfg = make_brgemv_ir_conf(brg);
...
struct jit_brgemv_ir_t : public jit_base_brgemm_kernel_t {
    DECLARE_CPU_JIT_AUX_FUNCTIONS(jit_brgemv_ir_t)

    jit_brgemv_ir_t(const brgemm_desc_t &abrg)
        : jit_base_brgemm_kernel_t(jit_name(), abrg.isa_impl), brg(abrg) {}
    ...
    void generate() override {
        ...
        const ir::reg_config_t reg_cfg = ir::make_reg_config(brg.isa_impl,
...
struct brgemv_ir_kernel_t : public brgemm_kernel_t {
    brgemv_ir_kernel_t(const brgemm_desc_t &abrd)
        : kernel_(new jit_brgemv_ir_t(abrd)) {}
    ~brgemv_ir_kernel_t() override { delete kernel_; }
...
private:
    jit_brgemv_ir_t *kernel_ = nullptr;
